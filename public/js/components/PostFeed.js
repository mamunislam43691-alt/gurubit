  ...
  async submitPost() {
    ...
    if (!data.success) {
      if (data.error?.code === 'LINK_DETECTED') {
        ...
        this._showSuccessToast('❌ ' + (data.error?.message || 'Failed to post'), 'error');
      }
      return;
    }
    this.showComposer = false;
