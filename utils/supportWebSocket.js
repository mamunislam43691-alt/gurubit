/**
 * WebSocket handlers for live support
 */
...
function handleSupportMessage(wss, ws, data) {
  switch (data.type) {
    case 'support_visitor_join': {
      const session = supportStore.getSession(data.sessionId);
      if (!session) return;
    ...
    case 'support_send': {
      ...
      const session = supportStore.getSession(sessionId);
      if (!session) return;

      const from = ws.isSupportAdmin ? 'admin' : 'visitor';
      const msg = supportStore.addMessage(sessionId, { from, text, imageUrl });
      const updated = supportStore.getSession(sessionId);
    ...
    case 'support_admin_join': {
      ...
      ws.send(JSON.stringify({
        type: 'support_admin_ready',
        sessions: supportStore.listSessions()
      }));
      break;
    }
...
function onSupportDisconnect(ws) {
  if (ws.supportSessionId) {
    supportStore.removeVisitorSocket(ws.supportSessionId);
  }
  if (ws.isSupportAdmin) {
    supportStore.removeAdminSocket(ws);
  }
}
